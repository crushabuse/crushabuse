...
app.get('/api/check/:hash', async (req, res) => {
    ...
    async function pull(page = 1) {
        const messages = await bolt.client.search.messages({ query: `https://zkcrush.xyz/crush?hash=${hash}`, count: 100, page })
        if (!messages.ok) return notok()
        messages.messages.matches.forEach(message => {
            if (message.text.includes(hash)) urls.push({
